...
template <typename Dtype>
void ORNConvolutionLayer<Dtype>::RotateARF_gpu() {
...
template <typename Dtype>
void ORNConvolutionLayer<Dtype>::AlignARF_gpu() {
  const int kH = this->kernelSize;
  const int nOrientation = this->nOrientation;
  const int nOutputPlane = this->nOutputPlane;
  const int nInputPlane = this->nInputPlane;
...
template <typename Dtype>
void ORNConvolutionLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
...
template <typename Dtype>
void ORNConvolutionLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
...
INSTANTIATE_LAYER_GPU_FUNCS(ORNConvolutionLayer);
...
template __global__ void RotateKernel<double>(
    ...
    const unsigned int nEntry,
    double* weight_data);
template void ORNConvolutionLayer<float>::RotateARF_gpu();
template void ORNConvolutionLayer<double>::RotateARF_gpu();
...
template __global__ void AlignKernel<double>(
    ...
    const unsigned int nEntry,
    double* weight_data);
template void ORNConvolutionLayer<float>::AlignARF_gpu();
template void ORNConvolutionLayer<double>::AlignARF_gpu();
}  // namespace caffe
